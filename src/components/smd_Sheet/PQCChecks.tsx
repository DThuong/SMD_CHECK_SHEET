...
import {
  fetchPQCCheck,
  updatePQCCheck,
  uploadPQCCheckIssueImage,
  deletePQCCheckImage,
  deletePQCCheckIssueImage,
...
const PQCChecks = memo(({ canEdit }: { canEdit: boolean }) => {
  ...
  useSubTableFetch(pqcCheckId, fetchPQCCheck);
  ...
  const handleRemoveImage = async (
    ...
    try {
      ...
      showNotification(
        "success",
        "Đã xóa",
        `Đã xóa ảnh ${field === "imgIC" ? "IC" : "vấn đề phát sinh"}`,
      );
    } catch (error) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      showNotification(
        "error",
        "Lỗi lưu PQC Checks",
        "Có lỗi xảy ra khi cập nhật pqc Checks",
      );
    }
  };
      ...
      <div className="p-0 py-4 w-full">
        ...
        {pqcCheckId && (
          ...
          >
            ...
            <span>
              PQCCheck ID: <strong>{pqcCheckId}</strong>
            </span>
            {currentSheet?.id && (
              <span>
          ...
          <table className="border border-gray-600 w-full text-center opacity-80">
            <tbody>
              ...
              <tr>
                <td
                  colSpan={1}
                  className="border border-gray-300 px-2 py-2 text-xs bg-gray-100 font-bold"
                >
                  {t("fields.imageIC")}
                </td>
                <td
          ...
          <div className="max-h-[60vh] overflow-y-auto scrollbar-hide">
            <div className="grid gap-3 p-1">
              ...
              <MultiImageUpload
                ...
                onViewAll={() =>
                  openImagePreview(form.imgIC || [], "Hình ảnh IC", 0)
                }
