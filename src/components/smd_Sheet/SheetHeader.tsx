...
const SheetHeader = memo(({ canEdit, returnPath }: SheetHeaderProps) => {
  ...
  const { lcrFileData, lcrValidation } = useAppSelector(state => state.fileSlice);
 ...
 const submit = async () => {
  ...
  if (!hasExistingLcr && !hasExistingReflow) {
    if (!tempFileState.lcr || !tempFileState.reflow) {
      showNotification('warning', 'Thiếu file', 'Lần đầu tiên phải upload cả 2 file: Excel và PDF.');
      return;
    }
  }

  // Nếu đã có file rồi → CHỈ upload file nào được chọn
  if (!tempFileState.lcr && !tempFileState.reflow && tempNoteFile === currentSheet.noteFile) {
    showNotification('warning', 'Chưa có thay đổi', 'Vui lòng chọn file mới để upload hoặc sửa ghi chú.');
    return;
  }
  ...
  try {
    ...
    const updatedWorkers = {
      ...
      reflowWorker: tempFileState.reflowWorker || existingWorkers.reflowWorker
    };
              ...
              <div className="min-w-0">
                ...
                {!lcrFileStatus.isValid && lcrFileStatus.stats && (
                  <div className="text-xs mt-1 space-y-1">
                    ...
                    {lcrFileStatus.stats.ng > 0 && (
                      <div className="text-red-600">NG: {lcrFileStatus.stats.ng}</div>
                    )}
                    {lcrFileStatus.stats.skip > 0 && (
                      <div className="text-orange-600">SKIP: {lcrFileStatus.stats.skip}</div>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="grid gap-4 max-h-[60vh] overflow-y-auto scrollbar-hide">
            ...
            <div className="p-3 border rounded-lg bg-gray-50">
              <h4 className="text-sm font-semibold mb-2 flex items-center gap-2">
                LCR file (xlsx)
                {tempFileState.lcr && <span className="text-green-600 text-xs">✓ Sẽ thay thế</span>}
                {!tempFileState.lcr && hasExistingLcr && <span className="text-blue-600 text-xs">→ Giữ nguyên</span>}
              </h4>
              ...
              <label className="text-xs block mb-2 w-full">
                ...
                <input
                  ...
                  onChange={(e) => setTempFileState(prev => ({ 
                    ...prev, 
              ...
              <label className="text-xs block mb-2 w-full">
                {hasExistingLcr ? 'Chọn file mới (tùy chọn):' : 'Chọn file mới:'}
                <input
                  type="file"
                  accept=".xlsx,.xls"
            ...
            <div className="p-3 border rounded-lg bg-gray-50">
              <h4 className="text-sm font-semibold mb-2 flex items-center gap-2">
                reflow file (pdf)
                {tempFileState.reflow && <span className="text-green-600 text-xs">✓ Sẽ thay thế</span>}
              ...
              <label className="text-xs block mb-2 w-full">
                {hasExistingReflow ? 'Chọn file mới (tùy chọn):' : 'Chọn file mới:'}
                <input
                  type="file"
                  accept=".pdf"
