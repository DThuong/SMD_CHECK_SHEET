...
const Dashboard = () => {
  ...
  const [shiftFilter, setShiftFilter] = useState<'morning' | 'night' | 'both'>('both');

  const { t } = useTranslation('dashboard');

  // Xác định 1 sheet thuộc ca nào, trả về "shift day" key
  const getShiftDay = (date: Date): { shift: 'morning' | 'night'; key: string } => {
    const h = date.getHours();
    if (h >= 8 && h < 20) {
      // Ca sáng: 08:00 - 19:59 → tính theo ngày hiện tại
      const y = date.getFullYear();
      const m = String(date.getMonth() + 1).padStart(2, '0');
      const d = String(date.getDate()).padStart(2, '0');
      return { shift: 'morning', key: `${y}-${m}-${d}` };
    } else {
      // Ca tối: 20:00 - 07:59 hôm sau → quy về ngày bắt đầu ca (ngày có 20:00)
      const shifted = new Date(date);
      if (h < 8) shifted.setDate(shifted.getDate() - 1); // 00:00-07:59 thuộc ca tối hôm trước
      const y = shifted.getFullYear();
      const m = String(shifted.getMonth() + 1).padStart(2, '0');
      const d = String(shifted.getDate()).padStart(2, '0');
      return { shift: 'night', key: `${y}-${m}-${d}` };
    }
  };
  ...
  const timelineStats = React.useMemo(() => {
    ...
    // Gộp tất cả keys, sort theo ngày
    const allKeys = [...new Set([
      ...Object.keys(morningCounts),
      ...Object.keys(nightCounts)
    ])].sort();

    return allKeys.map(key => {
      const [y, m, d] = key.split('-');
      const label = `${parseInt(d)}/${parseInt(m)}`;
      return {
        date: label,
        morning: morningCounts[key] || 0,
        night: nightCounts[key] || 0,
      };
    });
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 md:p-4 mb-4 mt-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
              ...
              <div className="flex gap-2 flex-wrap">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${timeRange === 'week' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${timeRange === 'month' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${timeRange === 'all' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}
                >
                ...
                <div className="flex gap-1 ml-2 border-l pl-2">
                  {(['both', 'morning', 'night'] as const).map(s => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${shiftFilter === s ? 'text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                        } ${shiftFilter === s && s === 'morning' ? 'bg-red-500' :
                          shiftFilter === s && s === 'night' ? 'bg-purple-600' :
                            shiftFilter === s && s === 'both' ? 'bg-blue-500' : ''
                        }`}
                    >
                      {s === 'both' ? 'Cả 2' : s === 'morning' ? 'Day Shift' : 'Night Shift'}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={timelineStats}>
                ...
                <XAxis dataKey="date" tick={{ fontSize }} stroke="#64748b" angle={-30} textAnchor="end" height={60} />
                <YAxis stroke="#64748b" />
                <Tooltip contentStyle={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px' }} />
                <Legend />
                {/* ✅ 2 Line thay vì 1, ẩn/hiện theo shiftFilter */}
                {(shiftFilter === 'morning' || shiftFilter === 'both') && (
                  <Line
                    type="monotone" dataKey="morning" stroke="#E24B4A" strokeWidth={2}
                    dot={{ fill: '#E24B4A', r: 4 }} activeDot={{ r: 6 }}
                    name="Ca sáng (08:00–19:59)"
                  />
                )}
                {(shiftFilter === 'night' || shiftFilter === 'both') && (
                  <Line
                    type="monotone" dataKey="night" stroke="#534AB7" strokeWidth={2}
                    dot={{ fill: '#534AB7', r: 4 }} activeDot={{ r: 6 }}
                    name="Ca tối (20:00–07:59)"
                  />
                )}
              </LineChart>
            </ResponsiveContainer>
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 md:p-4 mb-4 mt-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                onClick={() => setTimeRange('week')}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${timeRange === 'week'
              ...
              <button
                onClick={() => setTimeRange('month')}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${timeRange === 'month'
                  ? 'bg-blue-500 text-white'
                  : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
              >
              ...
              <button
                onClick={() => setTimeRange('all')}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${timeRange === 'all'
                  ? 'bg-blue-500 text-white'
                  : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
              >
